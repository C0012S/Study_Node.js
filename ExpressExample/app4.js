...
app.use(function(req, res, next) {
    ...
    res.writeHead('200', {"Content-Type":"application/json;charset=utf8"}); // 헤더 정보를 보낸다. // 200과 객체를 만들고 헤드 속성을 넣는다. // Content-Type이라는 속성을 넣어 준다. // 헤더 정보가 될 것이다. // text/html이라는 body 안에 들어가는 데이터의 타입을 넣어 줬다. 그것을 application/json이라고 한 번 넣어 줄 수도 있다. // → ch05-07. '200'으로 변경(변경 안 해도 정상적으로 동작함)
    // res.writeHead(200, {"Content-Type":"text/html;charset=utf8"}); // application/json 대신 text/html 넣어서 보내 본다. → 그래도 계속 오류 발생 → ch05-07. Content-Type 다시 application/json으로 변경
    res.write(personStr);
    // res.write(person); // res.write(person), res.write(personStr) 모두 정상적으로 처리하지 못하는 상태가 된다. 다시 변경해서 확인해 보기! // → ch05-07. res.write(personStr)로 변경
    res.end();
    // 이런 형태로도 보낼 수 있다.
...
// express를 이용해서 여러 개의 미들웨어를 만들었고 거기에서 응답을 보냈다. 그리고 그걸 JSON으로 보내는 경우를 봤다. // JSON으로 만들어서 보낸다는 것은, 자바스크립트 객체를 send를 이용해서 바로 보낼 수가 있고, 문자열로 바꿔서(JSON 문자열로 바꿀 때는, JSON.stringify를 쓰면 된다. / JSON 문자열을 다시 자바스크립트 객체로 바꿀 때는, JSON.parse 메소드를 쓰면 된다.) 보낼 수가 있다. 그리고 writeHead, end 방식을 그대로 사용할 수가 있다. 그러면 Content-Type을 application/json으로 하면 된다. 이런 식으로 바꿔서 보낼 수도 있다.
// 다 동일한 결과를 만들어 내는데 방법이 여러 가지가 있는 것이다.

// res 응답 객체에 status라고 정상이면 200 / 404, 403 같은 에러 코드를 넣을 수가 있다. // send로 내용을 보낼 수가 있다.
// 응답을 보낼 때, 응답 보내는 거 말고 redirect라는 다른 페이지로 이동하는 것이 있다. // redirect라는 건 어떻게 만드는지?
