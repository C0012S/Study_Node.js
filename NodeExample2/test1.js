...
console.dir(person); // 자바스크립트 객체를 그대로 출력해 준다. // 다른 사람이 만든 객체를 쓰는 경우(라이브러리를 쓰는 경우), 프로그램이 실행되는 중간에 만들어지는 경우 쓰면 장점이 된다. - 객체가 갖고 있는 속성을 알고 싶다면 dir로 객체를 그대로 뿌려 준다. 그럼 그 안에 들어간 속성들을 확인할 수 있다.
...
console.timeEnd('duration_time'); // time~timeEnd까지 걸린 시간을 체크해 준다. // console에 time과 timeEnd가 있어서 일정 시간을 체크할 수 있다. 어떤 코드까지의 걸린 시간을 확인할 수 있다.

console.log('파일 이름 : %s', __filename);
console.log('패스 : %s', __dirname);
// 어떤 파일이 어디에 있다는 정보를 확인할 때 지금처럼 __filename, __dirname이 사용된다. 이거를 전역 변수라고 한다.
// 어디서든 사용할 수 있는 객체를 전역 객체라고 하는데, 대표적인 게 console이다. 그리고 process 객체를 볼 수 있다.
